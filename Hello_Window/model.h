...
public:
    ...
    Model(string const &path,bool* isLoaded, bool gamma = false) : gammaCorrection(gamma)
    {
        loadModel(path, isLoaded);
    }
...
private:
    ...
    void loadModel(string const &path, bool* isLoaded)
    {
...
unsigned int TextureFromFile(const char *path, const string &directory, bool gamma)
{
    ...
    string edited_directory = directory;
    do
    {
        edited_directory.pop_back();
        i--;
    } while (directory[i] != '\\');

    filename = edited_directory + '/' + filename;
